    ...
    def build(self, train_data, config):
        ...
        for experiment_id, data in train_data['data'].items():
            ...
            saved_path = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))),
                                      'saved/model/cloud_ranger_runner/sock_shop/alpha_0_1/impact_graph')
            ...
            for i in range(len(impact_graph)):
                for j in range(len(impact_graph[0])):
                    if impact_graph[i][j] == 1:
                        G.add_edge(i, j)
            import matplotlib.pyplot as plt
            nx.draw(G, with_labels=True)
            graph_path = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))),
                                      'saved/myplot.png')
