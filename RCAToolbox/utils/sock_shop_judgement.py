...
class SockShopJudgement:
    def __init__(self):
        super().__init__()
        self.saved_path = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'saved', 'model')
        label_path = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'data', 'sock-shop')
    ...
    def load_experiment_result(self, model, alpha):
        self.experiment_result_dict = dict()
        saved_base_path = os.path.join(self.saved_path, model, 'score_ranking_list')
            ...
            if len(experiment_result) == 0:
                ...
                if 'node-cpu' in label['type']:
                    for i in range(min(5, len(experiment_result))):
                        m = list(experiment_result[i].keys())[0]
                        if 'service' in experiment_result[i].keys():
                            i -= 1
                            continue
                        if label['position'] in list(experiment_result[i].keys())[0] and 'CPU' in list(experiment_result[i].keys())[0]:
                            for j in range(i, 5):
                                ac[j] += 1
                            break
                if 'node-network' in label['type']:
                    for i in range(min(5, len(experiment_result))):
                        if 'service' in experiment_result[i].keys():
                            i -= 1
                            continue
                        if label['position'] in experiment_result[i].keys() and 'Network' in experiment_result[i].keys():
                            for j in range(i, 5):
                                ac[j] += 1
                            break
                if 'pod-pod' in label['type']:
                    for i in range(min(5, len(experiment_result))):
                        ...
                        if label['position'] in  list(experiment_result[i])[0]:
                            for j in range(i, 5):
                                ac[j] += 1
                            break
        avg = 0
...
if __name__ == '__main__':
    ...
    for i in [0.05, 0.1, 0.15]:
        sock_shop_judgement.load_experiment_result('monitor_rank_runner', 0.05)
